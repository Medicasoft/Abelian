...
function updateUser (req, res, next) {
    executeSql(req, res, next, 'User', 'update', [req.body.address.toLowerCase(), req.body.certificate, req.params.id], 200);
}
...
function deleteDomain(req, res, next) {
    ...
    db.query(get_qry, [req.params.id], function (err, result) {
        if (err) {
...
function executeSql(req, res, next, type, queryType, params, successCode) {
    ...
    db.query(qry, params, function (err, result) {
        if (err) {
            if(err.code === 'ER_DUP_ENTRY' && type.toLowerCase() === 'user' && err.message.indexOf('address_index') !== -1) {
                var outcome = {
        ...
        if ((queryType === 'update' || queryType === 'delete') && result.rows.affectedRows === 0) {
            res.send(404);
            return next();
        }

        if (queryType === 'create') {
            res.setHeader('location', baseUrl + type + '/' + result.rows.insertId);
        }
